$(function(){
	...
	(function(){
		...
		$lis.on('click',function(){
			window.location.href = '../rechargeRecordDetail.html'
		})
	})();
	//选择时间
	(function(){
		...
		var isStart = false;
		//finalDialog
		var $finalBtn = $('#content .datePicker .type');
		var $finalDialog = $('#finalDialog');
		var $finalLis = $finalDialog.find('li');
		var oY = 0;
		var oM = 0;
		var oD = 0;
		// day = 1;
		...
		$finalLis.on('click',function(e){
			$finalBtn.find('.final').html($(this).html());
			e.stopPropagation()
			$mask.hide();
			...
			if ($(this).index() == 0) {
				...
				if ((day-1) > 0) {
					...
					if (month == 1) {
						...
						oY = realYear;
						oM = month - 1;
						oD = getCalcDays(`${realYear}-${month-1}-${day}`);
					}
				}
				$btn.html(`${oY}-${oM}-${oD}`);
				$btn2.html(`${realYear}-${month}-${oD}`);
				...
				if (day>=7) {
					...
					if (month == 1) {
						...
						oY = realYear;
						oM = month-1;
						oD = getCalcDays(`${realYear}-${month-1}-${day}`)+day-6;
					}
				}
				setTime();
				...
				if (month == 1) {
					...
					oM = month-1;
					oD = 1;
					var abc =  getCalcDays(`${realYear}-${month-1}-${day}`);
					$btn.html(`${oY}-${oM}-${oD}`);
					$btn2.html(`${oY}-${oM}-${abc}`);
				}
			}
